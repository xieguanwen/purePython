...
if(__name__== "__main__"):
    ...
    endTime = nowTimestamp - 25*60
    startTime = nowTimestamp - 35*60

    db.cursor.execute(""" select * from shouji_order_info WHERE order_status = 0 AND pay_status=0 AND add_time>=%s AND add_time<=%s""",[startTime,endTime])
    ...
    for row in rows:
        ...
        if(smsRow is None):
            ...
            try:
                ...
                if(int(resp['alibaba_aliqin_fc_sms_num_send_response']['result']['err_code']) == 0):
